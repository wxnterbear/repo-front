...
app.use(express.json()); // Middleware para parsear peticiones en formato JSON
app.use(cors());
...
const rejectedIdeasFilePath = path.join(__dirname, 'rejected_ideas.json'); // Ruta al archivo de ideas rechazadas

// Función auxiliar para leer archivos JSON (cambio de fs.readFileSync a fs.readFile asíncrono)
const readJSONFile = async (filePath) => {
  const data = await fs.readFile(filePath, 'utf8'); // Uso de fs.readFile asíncrono
  return JSON.parse(data);
};

// Función auxiliar para escribir archivos JSON (cambio de fs.writeFileSync a fs.writeFile asíncrono)
const writeJSONFile = async (filePath, data) => {
  await fs.writeFile(filePath, JSON.stringify(data, null, 2), 'utf8'); // Uso de fs.writeFile asíncrono
};
...
// Obtener ideas aceptadas o rechazadas (cambio de filePath para usar las rutas correctas)
